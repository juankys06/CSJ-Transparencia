...
<%
	List<publicaciones> solicitudes = publicacionesLocalServiceUtil.getPorEstadoSolicitudes("SOLICITADO",0);
...
<div class="container-fluid">
	...
	<liferay-ui:error key="errorNegando" message="Error negando la solicitud" />
					...
					<liferay-ui:search-container   delta="10" deltaConfigurable="true"  emptyResultsMessage="No hay Solicitudes" iteratorURL="<%=iteratorURL%>">
							...
							<liferay-ui:search-container-row className="com.co.csj.service.model.publicaciones" keyProperty="id" modelVar="sol">
								<%
									...
									String nombre = datos_personales.getString("nombres")+" "+datos_personales.getString("primerApellido")+" "+datos_personales.getString("segundoApellido");
									nombre = nombre.replace("  ", " ");
								%>          	
							    <liferay-ui:search-container-column-text name="Funcionario" 		value="<%=nombre  %>"/>
							    <liferay-ui:search-container-column-text name="Codigo Despacho" 		value="<%=sol.getDespacho_usuario()  %>"/>
							    <liferay-ui:search-container-column-text name="Fecha Solicitud" 		value="<%=formato.format(sol.getFecha_solicitud())  %>"/>
							    <%
							    	String archivos = 	"<a href='?p_p_id=administracion_WAR_ley_transparenciaportlet&p_p_lifecycle=2&p_p_state=maximized&p_p_mode=view&p_p_cacheability=cacheLevelPage&doAsGroupId=19&refererPlid=10169&pagina=obtenerArchivo&id="+sol.getId()+"&archivo_solicitado=hoja_vida' target='_blank'>Hoja de Vida</a><br>"+
							    						"<a href='?p_p_id=administracion_WAR_ley_transparenciaportlet&p_p_lifecycle=2&p_p_state=maximized&p_p_mode=view&p_p_cacheability=cacheLevelPage&doAsGroupId=19&refererPlid=10169&pagina=obtenerArchivo&id="+sol.getId()+"&archivo_solicitado=declaracion_renta' target='_blank'>Declaracion de Renta</a><br>"+
							    ...
							    <liferay-ui:search-container-column-text name="Archivos" 				value="<%=archivos  %>"/>
					       		<liferay-ui:search-container-column-jsp  name="Acciones"  		path="/html/administracion/actionSolicitudesPendientes.jsp" />		     
