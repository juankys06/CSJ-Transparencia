<%@page import="com.co.csj.service.model.impl.correosImpl"%>
<%@page import="com.co.csj.service.service.correosLocalServiceUtil"%>
<%@page import="com.co.csj.service.model.correosClp"%>
...
<%
	correos corr = new correosImpl();
	String id = (String) renderRequest.getAttribute("id");
	if(id != null){
		corr = correosLocalServiceUtil.getcorreos(Long.parseLong(id));
	}
	...
	String entidad = "";
	String especialidad = "";
	if(corr.getCuentaCorreo().length()>10){
		if(corr.getCodigoDespacho().length()==12){
			departamento = corr.getCodigoDespacho().substring(0, 2);
			municipio = corr.getCodigoDespacho().substring(0, 5);
			entidad = corr.getCodigoDespacho().substring(5, 7);
			especialidad = corr.getCodigoDespacho().substring(7, 9);
...
<portlet:actionURL var="agregarFuncionario" name="agregarFuncionario"></portlet:actionURL>
	...
	<div style="display: inline-block; vertical-align: top; width: 85%;">
		<div class="container-fluid">
			<div class="row text-center">
				...
				<h3><%= id==null?"AGREGAR":"MODIFICAR" %></h3>
			</div>
			<br>
			<form action="<%=agregarFuncionario %>" method="post">
				<div class="row">
					<input type="hidden" name="id" value="<%=id%>">
					<div class="col-md-4">
						<div class="form-group">
							<label for="accion">Correo</label>
							<input class="anchoCompleto select-option inputsPanel" name="correo" id="correo" type="email" required="required" placeholder="ejemplo@cendoj.ramajudicial.gov.co" style="max-width: 100% !important;" pattern=".+@cendoj.ramajudicial.gov.co|.+@consejodeestado.gov.co|.+@consejosuperior.ramajudicial.gov.co|.+@corteconstitucional.gov.co|.+@cortesuprema.gov.co|.+@deaj.ramajudicial.gov.co|.+@notificacionesrj.gov.co|.+@ramajudicial.gov.co|.+@cortesuprema.ramajudicial.gov.co|.+@comisiondedisciplina.ramajudicial.gov.co" value="<%=corr.getCuentaCorreo() %>" >
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
							<label for="campo">Numero Documento</label>
							<input name="cedula" pattern="\d*" placeholder="123456789" class="anchoCompleto select-option inputsPanel" required="required" value="<%=corr.getCedulaResponsable() %>">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
							<label for="campo">Cargo</label>
							<input name="cargo" class="anchoCompleto select-option inputsPanel" placeholder="Juez" required="required"  value="<%=corr.getCargo() %>">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
							<label for="campo">Nombres</label>
							<input name="nombres" class="anchoCompleto select-option inputsPanel" placeholder="Pedro Pablo" required="required"  value="<%=corr.getNombre1() %>">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
							<label for="campo">Apellidos</label>
							<input name="apellidos" class="anchoCompleto select-option inputsPanel" placeholder="Perez Perez" required="required"  value="<%=corr.getApellido() %>">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
			    		...
			    		<label for="despacho" >Despacho:</label>
		      			<select class="anchoCompleto select-option inputsPanel" name="despacho" id="despacho" required="required" onchange="agregarCodigoDespacho()">
		      				...
		      				<% for(modeloBasico m : despa){ 
		      					if(corr.getCodigoDespacho().equalsIgnoreCase(m.getCodigo())){
		      				%>
		      					<option value="<%=m.getCodigo() %>" selected="selected"><%=m.getNombre() %></option>
		      				<%	}else{ %>
		      					<option value="<%=m.getCodigo() %>"><%=m.getNombre() %></option>
