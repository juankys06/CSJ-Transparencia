...
<%
	...
	JSONObject resultadoPA = JSONFactoryUtil.createJSONObject();
	
	if(renderRequest.getPortletSession().getAttribute("resultadoRPA")!=null){
		resultadoPA = (JSONObject) renderRequest.getPortletSession().getAttribute("resultadoRPA");
	}
...
<portlet:actionURL var="generarReportePA" name="generarReportePA"></portlet:actionURL>
	...
	<div style="display: inline-block; vertical-align: top; width: 85%;">
		<div class="container-fluid">
			<div class="row text-center">
				<h2>REPORTE PUBLICACIONES ACTIVAS</h2>
			</div>
			<br>
			<form action="<%=generarReportePA %>" method="post">
				<div class="row">
					<div class="col-md-4">
			...
			<% if(renderRequest.getPortletSession().getAttribute("resultadoRPA")!=null){ 
					...
					String ent = "";
					String esp = "";
					if(!resultadoPA.getString("departamento").isEmpty()){
						for(modeloBasico d : depa){
							if(d.getCodigo().equals(resultadoPA.getString("departamento")))
								dep = d.getNombre();
						}
					}
					if(!resultadoPA.getString("municipio").isEmpty()){
						List<modeloBasico> muni = consultas.getMunicipiosR(resultadoPA.getString("departamento"));
						for(modeloBasico d : muni){
							if(d.getCodigo().equals(resultadoPA.getString("municipio")))
								mun = d.getNombre();
						}
					}
					if(!resultadoPA.getString("entidad").isEmpty()){
						for(modeloBasico d : enti){
							if(d.getCodigo().equals(resultadoPA.getString("entidad")))
								ent = d.getNombre();
						}
					}
					if(!resultadoPA.getString("especialidad").isEmpty()){
						for(modeloBasico d : espe){
							if(d.getCodigo().equals(resultadoPA.getString("especialidad")))
								esp = d.getNombre();
						}
					}
			%>
			<div class="row">
				<div class="col-md-6">
					...
					Especialidad: <%=esp %>
					<br>
					Año de vigencia: <%= resultadoPA.getString("ano") %>
					<br>
					<br>
					<% 
						int cantidad = resultadoPA.getJSONArray("data").length();
					%>
					Cantidad de publicaciones:   <%=cantidad %>
				</div>
				<portlet:resourceURL var="ReporteExcel">
					<portlet:param name="pagina" value="2"/>
				</portlet:resourceURL>
...
$('#reportePublicacionesA').addClass("active");
